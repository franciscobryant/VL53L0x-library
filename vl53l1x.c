...
#define AddressDefault		UINT8_C(0x29 << 1)		// Use 8-bit address
...
{
  vl53l1x_writeReg(AddressDefault, new_addr & 0x7F);
  address = new_addr;
}
...
bool vl53l1x_init(bool io_2v8)
{
  ...
  // give it some time to boot; otherwise the sensor NACKs during the vl53l1x_readReg()
...
{
  I2C1_Write(AddressDefault, value, reg);
}
...
void vl53l1x_writeReg16Bit(uint16_t reg, uint16_t value)
{
  ...
  uint8_t buffer[2];
  buffer[0] = (value >> 8) & 0xFF; 		// value high byte
  buffer[1] = value       & 0xFF; 		// value low byte

  I2C1_WriteBuffer(AddressDefault, reg, I2C_MEMADD_SIZE_8BIT, buffer, 2);
}
...
void vl53l1x_writeReg32Bit(uint16_t reg, uint32_t value)
{
  ...
  uint8_t buffer[4];
  buffer[0] = (value >> 24) & 0xFF; 		// value high byte
  buffer[1] = (value >> 16) & 0xFF;
  buffer[2] = (value >> 8) 	& 0xFF;
  buffer[3] = value       & 0xFF; 		// value low byte
  I2C1_WriteBuffer(AddressDefault, reg, I2C_MEMADD_SIZE_8BIT, buffer, 4);
}
...
{
  uint8_t value;
  value = I2C1_Read(AddressDefault, reg);
  return value;
}
...
uint16_t vl53l1x_readReg16Bit(uint16_t reg)
{
  ...
  I2C1_ReadBuffer(AddressDefault, reg, I2C_MEMADD_SIZE_8BIT, buffer, 2);

  uint16_t value = (((buffer[0] & 0xff) << 8) | (buffer[1] & 0xff));
  return value;
}
...
uint32_t vl53l1x_readReg32Bit(uint16_t reg)
{
  ...
  I2C1_ReadBuffer(AddressDefault, reg, I2C_MEMADD_SIZE_8BIT, buffer, 4);

  uint32_t value = (((buffer[0] & 0xff) << 24) | ((buffer[1] & 0xff) << 16) | ((buffer[2] & 0xff) << 8) | (buffer[3] & 0xff));
  return value;
}
...
bool vl53l1x_setMeasurementTimingBudget(uint32_t budget_us)
{
  ...
  macro_period_us = calcMacroPeriod(vl53l1x_readReg(RANGE_CONFIG__VCSEL_PERIOD_A));
  ...
  vl53l1x_writeReg16Bit(MM_CONFIG__TIMEOUT_MACROP_A, encodeTimeout(
    timeoutMicrosecondsToMclks(1, macro_period_us)));

  // "Update Range Timing A timeout"
  vl53l1x_writeReg16Bit(RANGE_CONFIG__TIMEOUT_MACROP_A, encodeTimeout(
    timeoutMicrosecondsToMclks(range_config_timeout_us, macro_period_us)));

  // "Update Macro Period for Range B VCSEL Period"
  macro_period_us = calcMacroPeriod(vl53l1x_readReg(RANGE_CONFIG__VCSEL_PERIOD_B));

  // "Update MM Timing B timeout"
  // (See earlier comment about MM Timing A timeout.)
  vl53l1x_writeReg16Bit(MM_CONFIG__TIMEOUT_MACROP_B, encodeTimeout(
    timeoutMicrosecondsToMclks(1, macro_period_us)));

  // "Update Range Timing B timeout"
  vl53l1x_writeReg16Bit(RANGE_CONFIG__TIMEOUT_MACROP_B, encodeTimeout(
    timeoutMicrosecondsToMclks(range_config_timeout_us, macro_period_us)));
...
uint32_t vl53l1x_getMeasurementTimingBudget()
{
  ...
  // "Update Macro Period for Range A VCSEL Period"
  uint32_t macro_period_us = calcMacroPeriod(vl53l1x_readReg(RANGE_CONFIG__VCSEL_PERIOD_A));
...
//  Wire.requestFrom(address, (uint8_t)17);
  ...
  I2C1_ReadBuffer(AddressDefault, RESULT__RANGE_STATUS, I2C_MEMADD_SIZE_8BIT, buffer, 17);

//  results.range_status = Wire.read();
  results.range_status = buffer[0];

//	Wire.read(); // report_status: not used
//	buffer[1] is for report_status, we're not using it
...
//  results.dss_actual_effective_spads_sd0 |=           Wire.read();      // low byte
  results.dss_actual_effective_spads_sd0 = (((buffer[3] & 0xff) << 8) | (buffer[4] & 0xff));

//  Wire.read(); // peak_signal_count_rate_mcps_sd0: not used
//  Wire.read();
// 	buffer[5] and buffer[6] is for peak_signal_count_rate_mcps_sd0, we're not using it

//  results.ambient_count_rate_mcps_sd0  = (uint16_t)Wire.read() << 8; // high byte
//  results.ambient_count_rate_mcps_sd0 |=           Wire.read();      // low byte
  results.ambient_count_rate_mcps_sd0 = (((buffer[7] & 0xff) << 8) | (buffer[8] & 0xff));

//  Wire.read(); // sigma_sd0: not used
//  Wire.read();
// 	buffer[9] and buffer[10] is for sigma_sd0, we're not using it


//  Wire.read(); // phase_sd0: not used
//  Wire.read();
// 	buffer[11] and buffer[12] is for phase_sd0, we're not using it
...
//  results.final_crosstalk_corrected_range_mm_sd0 |=           Wire.read();      // low byte
  results.final_crosstalk_corrected_range_mm_sd0 = (((buffer[13] & 0xff) << 8) | (buffer[14] & 0xff));
...
//  results.peak_signal_count_rate_crosstalk_corrected_mcps_sd0 |=           Wire.read();      // low byte
  results.peak_signal_count_rate_crosstalk_corrected_mcps_sd0 = (((buffer[15] & 0xff) << 8) | (buffer[16] & 0xff));
...
bool checkTimeoutExpired() {
	return (io_timeout > 0) && ((uint16_t)(HAL_GetTick() - timeout_start_ms) > io_timeout);
};
